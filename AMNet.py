...
class AMNet(nn.Module):
    def __init__(self, in_channels: int = 3, num_classes: int = 2, base_c: int = 32, bilinear: bool = True):
        ...
        self.conv_input = DepthwiseSeparableConv(in_channels, base_c, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        if out3.size()[2:] != out5.size()[2:]:
            out5 = torch.nn.functional.interpolate(out5, size=out3.size()[2:], mode='bilinear', align_corners=False)
        ...
        if out2.size()[2:] != out6.size()[2:]:
            out6 = torch.nn.functional.interpolate(out6, size=out2.size()[2:], mode='bilinear', align_corners=False)
        ...
        if out1.size()[2:] != out7.size()[2:]:
            out7 = torch.nn.functional.interpolate(out7, size=out1.size()[2:], mode='bilinear', align_corners=False)
        ...
        if x.size()[2:] != out8.size()[2:]:
            out8 = torch.nn.functional.interpolate(out8, size=x.size()[2:], mode='bilinear', align_corners=False)
